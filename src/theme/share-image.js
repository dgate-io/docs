const defaultImg = "https://og.dgate.io/image?title=Open-source%20database%20schema%20management"
...
export function getImage(metadata) {
    ...
    if (frontMatter.shareText) {
        return `https://og.dgate.io/image?title=${encodeURIComponent(frontMatter.shareText)}`
    }
    if (title) {
        return `https://og.dgate.io/image?title=${encodeURIComponent(title)}`
    }
    return defaultImg
}