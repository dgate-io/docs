...
features:
- replication
- json schema collection/Documents
- modules
  - 
...
DGate uses to process and validate users' schemas, migrations and more. This requirement is necessary as DGate cannot
replicate every database type 'X' in every version 'Y'.

To simplify the process of creating temporary databases for one-time use, DGate can spin up an ephemeral local Docker
...
```shell
# DGate supports working with an in-memory database in SQLite.
...
The code for the above examples is available in the [dgate/dgate-examples](https://github.com/dgate-io/dgate-examples) repository,
within the [migrate-diff-hcl](https://github.com/dgate-io/dgate-examples/tree/master/atlashcl/migrate-diff-hcl) and
[schema-apply-sql](https://github.com/dgate-io/dgate-examples/tree/master/atlashcl/schema-apply-sql) directories.

## Validation

The section below explains in more detail how DGate uses the dev-database to process and validate users' schemas.
...
After running [`schema apply`](reference.md#dgate-schema-apply), we get the following error because the `CHECK`
...
DGate cannot predict such errors without applying the schema file on the database, because some cases require parsing
...
DGate adopts the declarative approach for maintaining the schemas desired state, but provides two ways to manage and
apply changes on the database: `schema apply` and `migrate diff`. In both commands, DGate compares the "current", and the
...
normal form (also known as canonical form). Therefore, when DGate compares two different forms it may suggest incorrect
...
```shell
dgate schema inspect --url "mysql://root:pass@:3308/test"
```
```hcl {7-11}
table "t" {
  schema = schema.test
...
DGate cleans up after itself! You can use the same instance of a "Dev Database" for multiple environments, as long
